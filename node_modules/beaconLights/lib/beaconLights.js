...
Beacon.prototype.newScan = function(rssi, txPower, IMMEDIATE_RANGE, NEAR_RANGE, ROLLING_AVERAGE_SIZE) {
    ...
    if (rangeAlt < IMMEDIATE_RANGE) {
        this.lights.red();
    } else if ( range > NEAR_RANGE ) {
        this.lights.green();
    } else if ( range <= NEAR_RANGE && range >= IMMEDIATE_RANGE ) {
        this.lights.amber();
    }
    this.resetTimer();
};
