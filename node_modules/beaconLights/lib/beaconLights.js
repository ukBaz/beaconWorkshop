...
var Beacon = function(uri, redPin, amberPin, greenPin) {
    this.uri = uri;
    this.txPower = 0;
    this.rssiValues = [];
    this.lights = new PiStop(redPin, amberPin, greenPin) ;
    this.timer = null;
    this.startTimer();
};
...
Beacon.prototype.newScan = function(rssi, txPower, CLOSE_RANGE, FAR_RANGE) {
    ...
    if (range < CLOSE_RANGE) {
        ...
        this.lights.green();
    } else if ( range <= FAR_RANGE && range >= CLOSE_RANGE ) {
        this.lights.amber();
    }
    this.resetTimer();
};
